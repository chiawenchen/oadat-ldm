...
import math
from models.AutoencoderKL_large import VAE

class LatentDiffusionModel(LightningModule):
    def __init__(self, config: TrainingConfig, noise_scheduler: DDIMScheduler, vae: VAE) -> None:
        super().__init__()
        self.config = config
        self.vae = VAE()
    ...
    def generate_fixed_noisy_images_for_variety_check(
        ...
        noises = torch.randn(
            (num_sampling, self.config.latent_channels, self.config.latent_size, self.config.latent_size), generator=self.generator, device=self.device
        )
    ...
    def generate_fixed_noisy_images(self, category: str) -> None:
        ...
        noisy_latens = []
        clean_latent = self.vae.encode(clean_image).latent_dist.sample()
        for idx, t in enumerate(timesteps):
            noisy_latent = self.noise_scheduler.add_noise(clean_latent, noises, t)
            noisy_latent_path = os.path.join(
            ...
            noisy_latens.append(noisy_latent)
    ...
    def denoise_and_save_samples(
        ...
        for t in range(timesteps.max(), -1, -1):  # From max timestep to 0
            ...
            if active_mask.any():
                active_noisy_images = denoised_latents[active_mask]
        ...
        denoised_images = self.vae.decode(denoised_latents).sample
