...
import torch
import numpy as np
from config.config import TrainingConfig, parse_arguments
...
def main() -> None:
    ...
    # Set up data module
    scd_small_indices = np.load("/mydata/dlbirhoui/chia/oadat-ldm/small_blob_scd_indices_700.npy")
    indices_swfd = np.load("/mydata/dlbirhoui/chia/oadat-ldm/train_sc_BP_indices.npy")
    indices_swfd = np.random.choice(indices_swfd, size=len(scd_small_indices), replace=False)
    datamodule = OADATDataModule(
        data_path=args.oadat_dir,
        batch_size=args.batch_size,
        num_workers=args.num_workers,
        mix_swfd_scd=args.mix_swfd_scd,
        indices_scd=scd_small_indices,
        indices_swfd=indices_swfd
    )

    # Set up noise scheduler
    noise_scheduler = get_named_beta_schedule(
        args.noise_schedule, config.num_train_timesteps
    )
    ...
    fixed_image_paths = {
        "swfd": os.path.join(config.sample_dir, "swfd_fixed_noisy_images"),
        "scd": os.path.join(config.sample_dir, "scd_fixed_noisy_images"),
    }
    os.makedirs(fixed_image_paths["swfd"], exist_ok=True)
